...
use crate::{
    error,
    providers::{aftonbladet, bbc, dn, nyt, svt, wsj, Post, Provider},
};
pub fn fetch_all_posts() -> Vec<Result<Vec<Post>, error::ParseRSSError>> {
    let r1 = thread::spawn(|| {
        aftonbladet::Aftonbladet::fetch_latest_posts(aftonbladet::AFTONBLADET_RSS)
    });
    ...
    let r6 = thread::spawn(|| bbc::Bbc::fetch_latest_posts(bbc::BBC_RSS));

    let mut outputs = vec![];

    for thread in Vec::from([r1, r2, r3, r4, r5, r6]) {
